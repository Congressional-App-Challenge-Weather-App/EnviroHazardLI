...
Generates realistic hazard dataset for training a neural network
to predict fire risk and flood risk for different towns.

Includes USGS/NOAA-style environmental data fields for consistency
with real-world data integration.
...
def generate_base_features(n_samples):
    """Generate base geographical and demographic features."""
    
    # Town names (repeated cyclically)
    town_names = [TOWN_NAMES[i % len(TOWN_NAMES)] for i in range(n_samples)]
    
    # Long Island coordinates (roughly)
    latitude = np.random.uniform(40.6, 41.2, n_samples)
    longitude = np.random.uniform(-73.5, -71.8, n_samples)
    
    # Population density (people per sq km)
    # Mix of rural (100), suburban (1000), and urban (5000) areas
    ...
    # Urbanization index (0-1, correlated with population density)
    ...
    # Elevation (meters) - Long Island is relatively flat but has variation
    elevation = np.random.gamma(2, 30, n_samples)  # Right-skewed distribution
    elevation = np.clip(elevation, 0.0, 2500.0)
    
    # Slope (degrees) - generally flat near coast, steeper inland
    slope = np.random.exponential(5, n_samples)
    slope = np.clip(slope, 0.0, 45.0)
    
    # Aspect (degrees, 0-360) - direction the slope faces
    aspect = np.random.uniform(0, 360, n_samples)
    
    # Distance to water (km) - Long Island is surrounded by water
    distance_to_water = np.random.exponential(5, n_samples)
    distance_to_water = np.clip(distance_to_water, 0.0, 50.0)
    
    # Impervious surface ratio (0-1) - correlated with urbanization
    ...
    # Vegetation density (0-1) - inversely correlated with urbanization
...
def generate_weather_features(n_samples):
    ...
    # Seasonal variation: some samples are hot, some cold
    ...
    # Precipitation last 24h (mm)
    # Most days are dry, occasional heavy rain
    ...
    # Precipitation last 7 days (mm) - should be >= 24h precipitation
    ...
    # Days since last rain (0-60)
    # If it rained in last 24h, days_since_rain should be 0
    ...
    # Storm warning flag (0 or 1)
    # More likely when high precipitation or high wind
    ...
    # Recent fire flag (0 or 1) - rare occurrence
...
def generate_usgs_noaa_features(df):
    """
    Generate USGS/NOAA-style environmental data fields.
    These are correlated with existing weather and geographical features.
    """
    
    # Soil moisture (0-1) - USGS-style soil moisture index
    # Higher with recent precipitation, lower with high temperature and dry days
    ...
    # Surface runoff (mm/hr) - depends on precipitation, impervious surface, slope
    # Higher precipitation and impervious surfaces increase runoff
    ...
    # Streamflow index (0-1) - USGS-style streamflow indicator
    # Higher with low elevation, proximity to water, and recent precipitation
    ...
    # Drought index (0-1) - NOAA-style drought severity index
    # Higher with high temperature, low precipitation, many dry days
...
def calculate_risk_scores(df):
    ...
    # Soil moisture (lower moisture = higher fire risk)
...
def calculate_hazard_class(fire_risk, flood_risk):
    """
    Determine overall hazard class based on both risk scores.
    Weighted combination: whichever is higher dominates.
    """
...
def generate_dataset(n_samples=200):
    ...
    # Generate USGS/NOAA-style environmental features
